...
export class object_space {
  static encode(message: object_space, writer: Writer): void {
    ...
    const field_zone = message.zone;
    if (field_zone !== null) {
      writer.uint32(18);
      writer.bytes(field_zone);
    }
...
export class caller_data {
  static encode(message: caller_data, writer: Writer): void {
    const field_caller = message.caller;
    if (field_caller !== null) {
      writer.uint32(10);
      writer.bytes(field_caller);
    }
...
export class compute_bandwidth_registry {
  static encode(message: compute_bandwidth_registry, writer: Writer): void {
    const field_entries = message.entries;
    for (let i = 0; i < field_entries.length; ++i) {
      writer.uint32(10);
      writer.fork();
      compute_bandwidth_entry.encode(field_entries[i], writer);
      writer.ldelim();
    }
  }
...
export class genesis_data {
  static encode(message: genesis_data, writer: Writer): void {
    ...
    for (let i = 0; i < field_entries.length; ++i) {
      writer.uint32(10);
      writer.fork();
      genesis_entry.encode(field_entries[i], writer);
      writer.ldelim();
    }
  }

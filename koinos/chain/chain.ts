...
import { koinos as koinos_2 } from "./koinos/common";
...
export namespace koinos {
  export namespace chain {
    ...
    export class head_info {
      static encode(message: head_info, writer: Writer): void {
        ...
        if (head_topology !== null) {
          writer.uint32(10);
          writer.fork();
          koinos_2.block_topology.encode(head_topology, writer);
          writer.ldelim();
        }
      ...
      static decode(reader: Reader, length: i32): head_info {
        ...
        while (reader.ptr < end) {
          ...
          switch (tag >>> 3) {
            case 1:
              message.head_topology = koinos_2.block_topology.decode(
                reader,
                reader.uint32()
              );
              break;
      ...
      head_topology: koinos_2.block_topology | null;
      head_block_time: u64;
      last_irreversible_block: u64;

      constructor(
        head_topology: koinos_2.block_topology | null = null,
    ...
    export class caller_data {
      ...
      caller_privilege: koinos.chain.privilege;

      constructor(
        caller: Uint8Array | null = null,
        caller_privilege: koinos.chain.privilege = 0
      ) {
        this.caller = caller;
    ...
    export class compute_bandwidth_registry {
      static encode(message: compute_bandwidth_registry, writer: Writer): void {
        ...
        for (let i = 0; i < entries.length; ++i) {
          writer.uint32(10);
          writer.fork();
          koinos.chain.compute_bandwidth_entry.encode(entries[i], writer);
          writer.ldelim();
        }
      }
          ...
          switch (tag >>> 3) {
            case 1:
              message.entries.push(
                koinos.chain.compute_bandwidth_entry.decode(
                  reader,
                  reader.uint32()
                )
              );
      ...
      entries: Array<koinos.chain.compute_bandwidth_entry>;

      constructor(entries: Array<koinos.chain.compute_bandwidth_entry> = []) {
    ...
    export class genesis_data {
      static encode(message: genesis_data, writer: Writer): void {
        ...
        for (let i = 0; i < entries.length; ++i) {
          writer.uint32(10);
          writer.fork();
          koinos.chain.genesis_entry.encode(entries[i], writer);
          writer.ldelim();
        }
      }
      ...
      static decode(reader: Reader, length: i32): genesis_data {
        ...
        while (reader.ptr < end) {
          ...
          switch (tag >>> 3) {
            case 1:
              message.entries.push(
                koinos.chain.genesis_entry.decode(reader, reader.uint32())
              );
              break;
      ...
      entries: Array<koinos.chain.genesis_entry>;

      constructor(entries: Array<koinos.chain.genesis_entry> = []) {
        this.entries = entries;
      }
    }

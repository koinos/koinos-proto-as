...
export class difficulty_metadata {
  static encode(message: difficulty_metadata, writer: Writer): void {
    ...
    if (field_target !== null) {
      writer.uint32(10);
      writer.bytes(field_target);
    }

    writer.uint32(16);
    writer.uint64(message.last_block_time);

    const field_difficulty = message.difficulty;
    if (field_difficulty !== null) {
      writer.uint32(26);
      writer.bytes(field_difficulty);
    }
...
export class get_difficulty_metadata_result {
  static encode(message: get_difficulty_metadata_result, writer: Writer): void {
    const field_value = message.value;
    if (field_value !== null) {
      writer.uint32(10);
      writer.fork();
      difficulty_metadata.encode(field_value, writer);
      writer.ldelim();
    }
  }
...
export class pow_signature_data {
  static encode(message: pow_signature_data, writer: Writer): void {
    ...
    const field_recoverable_signature = message.recoverable_signature;
    if (field_recoverable_signature !== null) {
      writer.uint32(18);
      writer.bytes(field_recoverable_signature);
    }
  }

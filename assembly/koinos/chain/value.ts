...
export class value_type {
  static encode(message: value_type, writer: Writer): void {
    const field_message_value = message.message_value;
    if (field_message_value !== null) {
      writer.uint32(10);
      writer.fork();
      Any.encode(field_message_value, writer);
      writer.ldelim();
    }
    ...
    const field_string_value = message.string_value;
    if (field_string_value !== null) {
      writer.uint32(122);
      writer.string(field_string_value);
    }

    const field_bytes_value = message.bytes_value;
    if (field_bytes_value !== null) {
      writer.uint32(130);
      writer.bytes(field_bytes_value);
    }
  }
  ...
  static decode(reader: Reader, length: i32): value_type {
    ...
    while (reader.ptr < end) {
      ...
      switch (tag >>> 3) {
        case 1:
          message.message_value = Any.decode(reader, reader.uint32());
          break;
...
export class list_type {
  static encode(message: list_type, writer: Writer): void {
    const field_values = message.values;
    for (let i = 0; i < field_values.length; ++i) {
      writer.uint32(10);
      writer.fork();
      value_type.encode(field_values[i], writer);
      writer.ldelim();
    }
  }
